...
@SuppressWarnings({ "PMD.UnusedPrivateField", "PMD.SingularField" })
    ...
    public void initialize() {
        ...
        _driveTrain.leftMaster.getTalon().getAllConfigs(_leftConfig);
        _driveTrain.rightMaster.getTalon().getAllConfigs(_rightConfig);
		...
		_rightConfig.remoteFilter0.remoteSensorDeviceID = _driveTrain.leftMaster.getTalon().getDeviceID(); // Device ID of Source
		...
		_driveTrain.leftMaster.getTalon().configAllSettings(_leftConfig);
        _driveTrain.rightMaster.getTalon().configAllSettings(_rightConfig);
        
        /* Configure output and sensor direction */
		_driveTrain.leftMaster.getTalon().setInverted(_leftInvert);
        _driveTrain.rightMaster.getTalon().setInverted(_rightInvert);

		/* Set status frame periods to ensure we don't have stale data */
		_driveTrain.rightMaster.getTalon().setStatusFramePeriod(StatusFrame.Status_12_Feedback1, 20, Constants.kTimeoutMs);
		_driveTrain.rightMaster.getTalon().setStatusFramePeriod(StatusFrame.Status_13_Base_PIDF0, 20, Constants.kTimeoutMs);
		_driveTrain.rightMaster.getTalon().setStatusFramePeriod(StatusFrame.Status_14_Turn_PIDF1, 20, Constants.kTimeoutMs);
		_driveTrain.rightMaster.getTalon().setStatusFramePeriod(StatusFrame.Status_10_Targets, 20, Constants.kTimeoutMs);
		_driveTrain.leftMaster.getTalon().setStatusFramePeriod(StatusFrame.Status_2_Feedback0, 5, Constants.kTimeoutMs);

		/* Initialize */
		_driveTrain.rightMaster.getTalon().setStatusFramePeriod(StatusFrameEnhanced.Status_10_Targets, 10);

        _driveTrain.leftMaster.getTalon().setSelectedSensorPosition(0, Constants.PID_PRIMARY, Constants.kTimeoutMs);
        _driveTrain.rightMaster.getTalon().setSelectedSensorPosition(0, Constants.PID_PRIMARY, Constants.kTimeoutMs);
        _driveTrain.leftMaster.getTalon().setSelectedSensorPosition(0, Constants.PID_TURN, Constants.kTimeoutMs);
        _driveTrain.rightMaster.getTalon().setSelectedSensorPosition(0, Constants.PID_TURN, Constants.kTimeoutMs);
		
        /* Determine which slot affects which PID */
        _driveTrain.rightMaster.getTalon().selectProfileSlot(Constants.kSlot_Distanc, Constants.PID_PRIMARY);
        _driveTrain.rightMaster.getTalon().selectProfileSlot(Constants.kSlot_Turning, Constants.PID_TURN);

        _targetAngle = _driveTrain.rightMaster.getTalon().getSelectedSensorPosition(1);
        _lockedDistance = _driveTrain.rightMaster.getTalon().getSelectedSensorPosition(0);
    ...
    public void execute() {
        ...
        _driveTrain.rightMaster.getTalon().set(TalonFXControlMode.MotionMagic, target_sensorUnits, DemandType.AuxPID, target_turn);
        _driveTrain.leftMaster.getTalon().follow(_driveTrain.rightMaster.getTalon(), FollowerType.AuxOutput1);    
    ...
    public boolean isFinished() {
        double error = _driveTrain.rightMaster.getTalon().getClosedLoopError();
